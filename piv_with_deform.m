function result = piv_with_deform(...
    ini_ti, fin_tf, roi_ini_ti, roi_fin_tf, x_img, y_img, ...
...
%
% Arguments:
%   ini_ti, fin_tf = 2D matrix, double, range 0 to 1, normalized grayscale image
%       from the start and end of the step to be analyzed.
%   roi_ini_ti, roi_fin_tf = 2D matrix, logical, mask indicating pixels where
...
validateattributes(roi_ini_ti, {'logical'}, {'2d', 'size', [nr, nc]});
validateattributes(roi_fin_tf, {'logical'}, {'2d', 'size', [nr, nc]});
...
% verbose output
if verbose
    fprintf('%s: ini: size = [%d, %d], roi frac = %.2f\n', mfilename, nr, nc, sum(roi_ini_ti(:))/numel(roi_ini_ti));
    fprintf('%s: fin: size = [%d, %d], roi frac = %.2f\n', mfilename, nr, nc, sum(roi_fin_tf(:))/numel(roi_fin_tf));
...
% craete sample grid
[r_grd_tm, c_grd_tm] = piv_sample_grid(samp_spc, size(ini_ti, 1), size(ini_ti, 2));

% initial guess for displacements and images at midpoint time
...
for pp = 1:np
    ...
    % NOTE: image deformation always assumes zero starting offsets
    [r_pts_tm, c_pts_tm, u_pts_tm, v_pts_tm] = piv_displacement(...
        ini_tm, fin_tm, r_grd_tm, c_grd_tm, zeros(size(r_grd_tm)), zeros(size(r_grd_tm)), ...

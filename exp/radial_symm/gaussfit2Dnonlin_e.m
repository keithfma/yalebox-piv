...
function [A, x0, y0, sigma_x, sigma_y, offset] = gaussfit2Dnonlin(z, tolz, params0, LB, UB, lsqoptions)
...
%     params0 = [min(z(:)), params0_2, params0_3, min([nx ny])/4, max(z(:))-min(z(:))];
    params0 = [min(z(:)), params0_2, params0_3, min([nx ny])/4, min([nx ny])/4, max(z(:))-min(z(:))];
end
...
%     UB = [max(z(:)),nx,ny,max([nx ny]),inf];
    UB = [max(z(:)),nx,ny,max([nx ny]),max([nx ny]),inf];
end
...
offset = params(1);
    ...
    function resids = objfun(params,px,py,z)
        xx = px(:) - params(2);
        yy = py(:) - params(3);
        pred = params(1) + params(6)*exp(-(xx.^2/2/params(4)^2+yy.^2/2/params(5)^2));

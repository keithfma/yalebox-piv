...
function [A, x0, y0, sigma_x, sigma_y, theta, offset] = gaussfit2Dnonlin_e_r(z, tolz, params0, LB, UB, lsqoptions)
...
%     params0 = [min(z(:)), params0_2, params0_3, min([nx ny])/4, max(z(:))-min(z(:))];
    params0 = [min(z(:)), params0_2, params0_3, min([nx ny])/4, min([nx ny])/4, max(z(:))-min(z(:)), pi/4];
end
...
%     UB = [max(z(:)),nx,ny,max([nx ny]),inf];
    UB = [max(z(:)),nx,ny,max([nx ny]),max([nx ny]),inf, 2*pi];
end
...
theta = params(7);
    ...
    function resids = objfun(params,px,py,z)
        xx = px(:) - params(2);
        yy = py(:) - params(3);
        a = cos(params(7))^2/2/params(4)^2 + sin(params(7))^2/2/params(5)^2;
        b = sin(2*params(7))/4/params(4)^2 + sin(2*params(7))/4/params(5)^2;
        c = sin(params(7))^2/2/params(4)^2 + cos(params(7))^2/2/params(5)^2;
        pred = params(1) + params(6)*exp(-(a*xx.^2 - 2*b*xx.*yy + c*yy.^2));

function opt = plot_displ_strain(piv_file, index, varargin)
% function opt = plot_displ_strain(piv_file, index, varargin)
%
% MODIFIED FOR BOSTON FUSION TALK, HELP NOT NEC. ACCURATE
% ULIM -> deformation limits
%
...
[displacement,spin,Dv,Dd,D2x,D2y,WkStar,AkStar] = deformation(xg,yg,uu,vv,~isnan(uu));
...
format_axes(gca, opt, 0, 1, opt.mlim, ...
    sprintf('Velocity Magnitude, step = %.1f', step));

% plot strain 2-invariant 
axes(ax_mid);
imagesc(xx, yy, Dd, 'AlphaData', ~isnan(Dd)); 
format_axes_strain(gca, opt, 1, 1, opt.ulim, ...
    sprintf('Strain (2nd Invariant), step = %.1f', step));

%...nothing in bottom axis
...
function [] = format_axes_strain(ax, opt, xshow, yshow, clim, title_str)
% apply consistent formatting to axes

function [Jp2w,Jw2p,xw0,yw0] = getpivwoco(imfile,xspt,sptloc)
% function [Jp2w,Jw2p,xw0,yw0] = getpivwoco(imfile,xspt,sptloc)
%
...
% solution for the transformation parameters.  The special characterisitics
% of the Yalebox coordinate system allow a few convienient assumptions,
% namely:
%
% 1) Images are rotated such that the bed is horizontal prior to running
% sandboxpiv, as a result the world coordinate system is not rotated
% relative to the ppixel coordinate system. Additionally, lens and
% perspective distortion is corrected so that "shearing" is also
% negligible.  In the Jacobian, off-diagonals = 0.
%
% 2) Images are cropped at the bottom of the sand, as a result the bottom
% row of pixels lies at yW = 0. A y-offset is applied to make this true.
%
% As a result of these special characteristics and assumptions, the world
% coordinate grids are regular (plaid) and the lower edge of the data lies
% at the lower edge of the matrix, which is useful for computations.
%
...
%
% Inverse transformation from world tp pixel coordinates is computed as:
%
...
% than lined up with the front of the centerpeice as it should be.  No
% default.
%
% sptloc = String.  Idetifies the location of the spoint in the image.
% Needed to get the proper vertical s-point adjustment value.  Valid
% choices are 'top' or 'bot', anything else whill return an error. No
% default.
%
% Returns:
%
...
try
    ...
    if ~strcmp(sptloc,'top') && ~strcmp(sptloc,'bot')
        fprintf(2,'ERROR: Bad value for sptloc\n');
        return
    end
    ...
    % choose yw0 so that the top row of pixels lies at yW = 0
    switch sptloc
        case 'top'
            yw0 = -(1*Jp2w(2,2));
        case 'bot'
    ...
    % Adjust parameters based on assumptions
    fprintf('PIV-specific assumptions:\n');
    fprintf('Dropping off-diagonal components of the Jacobian matrix: \n[%.2e\t%.2e \n %.2e\t%.2e]\n\n',Jp2w(1,1),Jp2w(1,2),Jp2w(2,1),Jp2w(2,2));
    fprintf('Adjusting y-offset: \nyw0 = %.6e -> %.6e, percent change = %.2f\n',yw0fit,yw0,100*(yw0fit-yw0)/yw0);
    Jp2w(1,2) = 0;
    Jp2w(2,1) = 0;
    
    % compute world-to-pixel transformations (inverse of the above, inverse of a diagnoal matrix is 1/elements)
    Jw2p = [1/Jp2w(1,1), 0; 0, 1/Jp2w(2,2)];
    ...
    % Save results
    save('getpivwoco_output.mat','Jp2w','Jw2p','xw0','yw0');

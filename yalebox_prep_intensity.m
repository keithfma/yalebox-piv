...
%
% Note: The modified adapthisteq function requires several mex files that
% reside in the image processing toolbox "private" folder. As such, they
% are NOT visible unless copied somewhere on the MATLAB path. I handle this
% by copying the files to a local "private" folder. The required files are:
%
%   - grayxformmex.mexa64 (R2014b) / grayxformmex.mexa64 (R2014a)
%   - imhistc.mexa64
%
% Arguments:
%
...
% (Keith Ma: deal with version dependant private function names )
if verLessThan('matlab', '8.4')
    grayxformmex = @grayxform;
end
...
for k=1:numTiles(1)+1
  ...
  for l=1:numTiles(2)+1
    ...
    imgPixVals = grayxformmex(I(imgTileIdx{1},imgTileIdx{2}), aLut);
    ...
    claheI(imgTileIdx{1}, imgTileIdx{2}) = ...
        (rowRevW .* (colRevW .* double(grayxformmex(imgPixVals,ulMapTile)) + ...
                     colW    .* double(grayxformmex(imgPixVals,urMapTile)))+ ...
         rowW    .* (colRevW .* double(grayxformmex(imgPixVals,blMapTile)) + ...
                     colW    .* double(grayxformmex(imgPixVals,brMapTile))))...
        /normFactor;

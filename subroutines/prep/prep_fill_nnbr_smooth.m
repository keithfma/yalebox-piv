...
for cc = 1:3
    ...
    % replace boundary pixels with smoothed values
    sband = band;
    for kk = unique(nbr_idx)
        [ii, jj] = ind2sub(size(mask_fill), kk);
        sband(ii, jj) = get_value(band, mask_fill, weight, ii, jj);
    end
    
    band(~mask_fill) = sband(nbr_idx);
    img_fill(:, :, cc) = band;
end
...
function val = get_value(img, msk, wgt, ii, jj)
% Return weighted image value at index (ii, jj), taking care to handle
%   NaN-padded edges
% %
dd = (size(wgt,1)-1)/2;
...
img_win = img((ii-dd):(ii+dd), (jj-dd):(jj+dd));
